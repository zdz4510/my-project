<template>
  <div>
		...
		<el-row :gutter="20" class="bgw">
				...
				<div>
					<el-select v-model="value" clearable placeholder="请选择" :disabled="selectIsDisabled" @clear="handleClearSelect" @change="handleChangeOption" @focus="handleSelectFocus" ref="select" >
						<el-option
							...
							<span style="float: right; color: #8492a6; font-size: 13px; margin-right:16px">{{ item.workCenterRelation }}</span>
						</el-option>
					</el-select>
			...
			<el-col :span="18">
				<div>
					<el-form :inline="true" :model="editForm" ref="editForm" :rules="rules" class="form-style" :label-width="formLabelWidth">
...
import {addStation, getOperationInfo} from '../../../api/operation.station.api.js'
...
export default {
	...
	methods: {
		...
		handleChangeOption(row) {
			...
			//过滤数组
			const tempList = this.cloneList.filter(item => item["operation"] == row.split('&')[0] && item['workCenterRelation'] == row.split('&')[1] && item['station'] == row.split('&')[2] );
