<template>
  <div class="materialListEdit">
    <div class="operate">
      ...
      <dsn-button size="small" type="primary" @click="checkAdd(['baseInfoOne', 'baseInfoTwo'])">保存</dsn-button>
      <dsn-button
        size="small"
        type="primary"
      ...
      <div class="right">
        ...
        <el-tabs type="border-card">
          <el-tab-pane class="baseInfo">
            ...
            <el-form :model="baseInfo" ref="baseInfoTwo" label-width="150px" :rules="rules">
          ...
          <el-tab-pane class="materialDetail">
            ...
            <dsn-button size="small" type="primary" @click="handleInsert">插入</dsn-button>
            <dsn-button
              size="small"
              type="danger"
              icon="el-icon-delete"
            ...
            <div class="showInfo">
              ...
              >
                ...
                <el-table-column label="组件描述">
                  <template slot-scope="scope">
                    <dsn-input
                      :value="getMaterialInfo(scope.row.material, 'materialDes')"
                      disabled
                    />
                  </template>
                </el-table-column>
                ...
                <el-table-column label="组件类型">
                  <template slot-scope="scope">
                    <dsn-input
                      :value="getMaterialInfo(scope.row.material, 'materialType')"
                      disabled
                    />
                  </template>
                </el-table-column>
...
export default {
  data() {
    const valiBomRev = (rule, value, callback) => {
      ...
      if (!reg.test(value)) {
        callback("版本号格式应只包含（[A-Z,0-9,_,-,/]）");
      }
      this.baseInfo.bomRev = this.baseInfo.bomRev.toUpperCase();
      callback();
    };
    return {
      valiBomRev,
  ...
  created() {
    ...
    if (this.operateType === "edit") {
      this.baseInfo =
        this.cloneList[0] ||
        JSON.parse(localStorage.getItem("materialList"))[0];
  ...
  methods: {
    ...
    getMaterialInfo(data, type) {
      ...
      return (
        materialAll &&
        materialAll.find(
          item => `${item.material}/${item.materialRev}` === data
        )[type]
      );
    },
    ...
    handleDelete() {
      ...
      this.bomMaterialList = this.bomMaterialList.filter(
        item =>
          !selectionList.some(
            ele =>
              ele.material === item.material && ele.sequence === item.sequence
          )
      );
    ...
    handleSave() {
      const params = {};
      const { operateType, baseInfo, bomMaterialList } = this;
      if (operateType === "add") {
