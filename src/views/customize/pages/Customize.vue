<template>
    ...
    <el-dialog title="添加/编辑" :visible.sync="dialogVisible">
      <el-form :inline="true" :model="addSetForm" ref="addSetForm" :rules="srules" class="form-style" label-position="right" :label-width="formLabelWidth">
        <el-row>
					<el-col :span="12">
    ...
    created(){
        ...
        this.$refs[formName].validate((valid) => {
          if (valid) {
            ...
            if(this.currentOperation == 'add'){
              if(this.SetupInfoList.findIndex(item => JSON.parse(JSON.stringify(item.fieldName)) == JSON.parse(JSON.stringify(form.fieldName))) == -1){
      ...
      delSet(){
          ...
          this.sCheckedList.map((val=>{
            this.SetupInfoList.splice(this.SetupInfoList.findIndex(item => item.parameter === val.parameter), 1)
          }))
          this.$refs.sTable.clearSelection()
        }).catch(() => {
          this.$message({
