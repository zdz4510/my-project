        ...
        <el-row>
          <el-col :span="8">
            <el-form-item label="用户:" prop="user">
              <dsn-input v-model="addForm.user" @change="blurSearchFor"></dsn-input>
            </el-form-item>
          </el-col>
          <el-col :span="8">
                ...
                <el-row>
                  <el-col :span="24">
                    <el-row>
                      <el-col :span="8">
                        <dsn-table
                          :data="allocateData.filter(data => !cert1 || data.cert.toLowerCase().includes(cert1.toLowerCase()))"
                          @select="check1"
                          @select-all="check1"
                        >
                          ...
                          <el-table-column label>
                            <template slot="header">
                              <dsn-input v-model="cert1" placeholder="输入上岗证搜索" />
                            </template>
                      ...
                      <el-col :span="8">
                        <dsn-table
                          :data="unallocateData.filter(data => !cert2 || data.cert.toLowerCase().includes(cert2.toLowerCase()))"
                          @select="check2"
                          @select-all="check2"
                        >
                          <el-table-column label="上岗证:">
...
<script>
import { saveData, getAllCert, checkUser } from "../../../api/cert.user.api";
    ...
    save(formName) {
      this.$refs[formName].validate(valid => {
        if (valid) {
          ...
          saveData({ createList: [params], updateList: [] }).then(data => {
